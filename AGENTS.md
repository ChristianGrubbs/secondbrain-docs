...
| `vault-capture-e2e.test.ts` | Fork-local: Task 6 (6A/6B) format/behavior qualification against the built executable and a real `obsidian-cli` over a throwaway vault — 20 required-format rows (F01-F20, four network-dependent and skipped without connectivity), real-CLI first-capture directory bootstrapping (C01-C04: no inbox, no Doc Sets parent, Unicode/spaces collection name, mixed-case reuse), a process-level SIGINT cancellation row (X03, exit 130), and subprocess/vault-access count + elapsed-time measurements (M01-M02); asserts the live vault is untouched. See `docs/migration-qualification.md` for the row-by-row report. Since 2026-09-14: F06 proves local-asset copy+link through `obsidian-cli attach`, F07 asserts the Markdown table from a ruled-table fixture, and the D03 describe asserts no default-config creation or overwrite. | prior `npm run build`; `~/ai-stack/bin/obsidian-cli`; skips otherwise; network for the four live rows | yes (skips if absent; live rows skip without network) |
