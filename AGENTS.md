...
| `vault-index-e2e.test.ts` | Fork-local: `sb-docs capture`/`search`/`read`/`reindex` against the built executable and a throwaway vault; pending indexing under a held index lock still exits 0, `search`/`reindex` exit nonzero and keep the prior generation, and a body phrase is retrievable while the frontmatter content hash is not | prior `npm run build`; `~/ai-stack/bin/obsidian-cli`; skips otherwise | yes (skips if absent) |
