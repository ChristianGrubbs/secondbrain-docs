...
- `src/scraper/fetcher/FingerprintGenerator.ts` (and its test) — DOM settle rev 3 (2026-09-15), intentional shared behavior change: the default header pool is desktop-only (`devices: ["desktop"]`, `operatingSystems: ["windows", "linux", "macos"]`); callers may still pass mobile options explicitly. Both `HttpFetcher` and `BrowserFetcher` draw from this generator, and a randomly drawn mobile user agent made Salesforce Experience Cloud serve a `formFactor: SMALL` bootstrap that never rendered in headless desktop Chromium, publishing the raw shell. This removes the random mobile draw; it does not make the browser fingerprint fully self-consistent (a Firefox or Safari header set can still accompany Chromium rendering). Pinned by `FingerprintGenerator.test.ts` (constructor constraints asserted through a `header-generator` spy, 100 sampled user agents, mobile negative control) and no-mobile-UA sampling in `HttpFetcher.test.ts` / `BrowserFetcher.test.ts`.
