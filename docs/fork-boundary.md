...
| `src/vault/VaultPublisher.ts` | `VaultPublisher` — create / unchanged / replace (CAS) / conflict publication of one source note plus one MOC link, recovery of interrupted journal entries; `hasLinkTo({ markdown, target })` delegates to the shared, `remark`-based `src/vault/markdownLinks.mjs`; packet 15 (2026-09-14) localizes `file://` image embeds before rendering and attaches every document-local asset through `ObsidianCli.attachFile` before the note write (create, replace and unchanged paths), reporting `Publication.attachments`; since 2026-09-15 the replace path compares the prior note's frontmatter title with the incoming one and, when they differ, rewrites only that note's existing MOC line alias under CAS (`readNoteWithAnchor` / `replaceNote --if-match`, one retry, then a logged skip) — `unchanged` publications, same-title replaces and human-edited aliases on an unchanged title never touch the index, and note paths stay frozen by `source_id` |
