...
| `src/vault/sourceBody.ts` | `stripLeadingFrontmatter` / `isMarkdownContentType` — drops the frontmatter block a Markdown source served (valid YAML or not) before publication, so the note carries exactly one frontmatter block, the publisher's own (2026-09-15 skill test: higgsfield.ai) |
| `src/vault/sourceBody.test.ts` | Unit contract: valid and malformed leading blocks, CRLF, block not at the start, thematic break, unclosed block, frontmatter-only document; content-type matching |
