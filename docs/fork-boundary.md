...
| `src/vault/VaultPublisher.ts` | `VaultPublisher` — create / unchanged / replace (CAS) / conflict publication of one source note plus one MOC link, recovery of interrupted journal entries; `hasLinkTo({ markdown, target })` delegates to the shared, `remark`-based `src/vault/markdownLinks.mjs` |
