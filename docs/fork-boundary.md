...
| `src/vault/VaultPublisher.ts` | `VaultPublisher` — create / unchanged / replace (CAS) / conflict publication of one source note plus one MOC link, recovery of interrupted journal entries; `hasLinkTo({ markdown, target })` delegates to the shared, `remark`-based `src/vault/markdownLinks.mjs`; packet 15 (2026-09-14) localizes `file://` image embeds before rendering and attaches every document-local asset through `ObsidianCli.attachFile` before the note write (create, replace and unchanged paths), reporting `Publication.attachments`; since 2026-09-15 a conflict candidate is written as `00 Inbox/Source Capture Updates/<sanitized title> <sourceId[0:12]>-<semanticDigest[0:12]>.md` via `candidateFilename` (readable name, still content addressed: an unchanged conflict reuses one candidate and a human-edited candidate is still detected by the journal digest); since 2026-09-15 the replace path compares the prior note's frontmatter title with the incoming one and, when they differ, rewrites only that note's existing MOC line alias under CAS (`readNoteWithAnchor` / `replaceNote --if-match`, one retry, then a logged skip) — `unchanged` publications, same-title replaces and human-edited aliases on an unchanged title never touch the index, and note paths stay frozen by `source_id` |
| `src/vault/VaultPublisher.test.ts` | Unit contract for the publisher, including MOC-link regressions for backtick-fenced, tilde-fenced, inline-code-span, 1-3-space-indented-fence, multi-backtick-span, multiline-span and mismatched-delimiter-run mentions not counting as live links, plus bogus-closing-fence and fence-looking-inline-text real links not being duplicated |
| `src/vault/markdownLinks.mjs` | `countLinksTo({ markdown, target })`/`hasLinkTo({ markdown, target })` — the one shared Markdown wikilink-detection implementation, used by both `VaultPublisher.ts` and `scripts/lib/qualification-contract.mjs`. Round 6 (scoped) replaced the round-5 hand-rolled line-based fence/inline-span stripper (not CommonMark-correct) with a real parse via the `remark`/`remark-parse`/`unified` toolchain already in the dependency tree (also used by `src/splitter/SemanticMarkdownSplitter.ts`; no new dependency), walked via a small local `walk()` (not `unist-util-visit`, which is only a transitive dependency, not declared in `package.json`). Round 6's third pass REMOVED a brief substring fast path entirely (it ignored Markdown escapes like `\-` and character references like `&amp;` that remark resolves differently from the raw bytes, which could make the publisher insert a duplicate note) -- `countLinksTo`/`hasLinkTo` now always parse; the single-entry parse cache (correctness-preserving) is kept. That same pass also replaced an in-band Unicode Private Use Area boundary character (`U+E000`) with a structural design: `collectVisibleTextRuns`/`textRunsOf` build an ARRAY of independent text runs, splitting at every opaque node (`code`, `inlineCode`, `break`, `image`, `imageReference`, `html`, `footnote`, `footnoteReference`) and joining only transparent children (emphasis, strong, link) into the current run -- no separator character of any kind, so a real vault path containing that exact code point cannot collide with anything. Root-level (block) `html` is deliberately never scanned -- `renderCollectionIndex` in `src/vault/render.ts` never emits it, so a link inside raw block HTML is out of scope, the same as inside a code fence. Round 9 (third scoped re-review) fixed the wikilink regex's alias group, which stopped at the first `]`: an alias containing a literal `]` (a backslash escape or an HTML character reference, both resolved by remark) made the whole link fail to match; the alias group became `(?:(?!\]\]).)*` (matches any character, including a lone `]`, stopping only right before the real closing `]]`). Round 10 (fourth scoped re-review) found that same alias group never stopped at a NESTED `[[` either, so an unterminated link could "borrow" a later link's closing `]]` and wrongly count itself as already-linked -- fixed by replacing the regex-based matching entirely with `countWikilinksInRun`, an explicit linear two-pointer scanner (find `[[`, find the next `]]`, reject as malformed if a nested `[[` occurs first, compare target/alias as plain strings). Root-caused the reported performance regression to `remark-parse`/`micromark`'s own CommonMark bracket-resolution algorithm having superlinear cost on pathologically bracket-dense malformed input (verified via isolated `processor.parse()` timing) -- not this module's counting logic, which is linear; a real, publisher-authored MOC can never reach that pathological shape. Round 11 (fifth scoped re-review, BLOCKER) found `countWikilinksInRun` itself was NOT actually linear: it re-ran `indexOf` from the current position on every iteration, so `"[[".repeat(n) + "valid]]"` made every nested opener re-scan toward the same distant closer (quadratic). Rewritten as a genuinely single-pass scan with one monotonically-advancing index and at most one tracked "current opener" -- verified sub-1ms at 256KB for the exact shape that previously took ~273ms. |
| `src/vault/markdownLinks.test.ts` | Unit contract for `countLinksTo`/`hasLinkTo`: plain/aliased link counting, the plain-text-syntax probe, emphasis-split reassembly, the 8 named CommonMark fence/span edge cases, duplicate-link counting, opaque-node fragmentation regressions (inline code, hard break, image, imageReference, inline HTML), the block-HTML scope decision, structural coverage (headings, list items, blockquotes, a pipe-table line, strong/link reassembly, cross-block non-merging), Markdown-escape/character-reference targets, a target containing the former sentinel character (U+E000), a cache-sequence regression, and a benchmark-style large-flat-list-MOC performance test documenting the always-parse cost |
...
| `src/vault/ObsidianCli.ts` | Argument-array subprocess wrapper over `obsidian-cli`, note bytes on stdin; both missing-note and both missing-directory diagnostics read as absent; `attachFile(localPath, vaultPath)` wraps `obsidian-cli attach ... --force` (the one sanctioned binary write, ai-stack #638) and `attach` is classified as a write |
...
| `src/vault/identity.ts` | `source_id`, collection paths, filename and link sanitisation; `candidateFilename` builds the byte-capped `<sanitized title> <sourceId[0:12]>-<semanticDigest[0:12]>.md` conflict-candidate basename |
| `src/vault/sourceBody.ts` | `stripLeadingFrontmatter` / `isMarkdownContentType` — drops the frontmatter block a Markdown source served (valid YAML or not) before publication, so the note carries exactly one frontmatter block, the publisher's own (2026-09-15 skill test: higgsfield.ai). A block counts as frontmatter only with an opening and closing `---` line, no blank line between them, and at least one `key:` line, so a leading thematic break can never be "closed" by a later `---` in prose or a code fence (Codex r1). Markdown types come from `MimeTypeUtils.isMarkdown` (`text/markdown`, `text/x-markdown`, `text/mdx`, `text/x-gfm`) |
| `src/vault/sourceBody.test.ts` | Unit contract: valid and malformed leading blocks, CRLF, block not at the start, thematic break alone / followed by another / followed by a fence containing `---`, rule followed by prose, two consecutive blocks (exactly one removed), unclosed block, frontmatter-only document; all four content types with parameters and case, plus negatives |
| `src/vault/localAssets.ts` | `localizeLocalAssets` — parser-based (`remark-parse` mdast `image`/`definition` nodes, never regex) rewrite of a `file://` source's relative Markdown image embeds to `_attachments/<collection folder>/<source_id[0:12]>/<path relative to the source>` plus the asset manifest the publisher copies first (Task 6 row F06) |
| `src/vault/localAssets.test.ts` | Unit contract for `localizeLocalAssets`: rewrite + manifest, untouched remote/absolute/`data:`/`.md`/missing targets, non-`file:` sources, dedupe and percent-decoding, directory-named target, image-looking text inside fenced/indented/inline code left alone, angle-bracket destinations with spaces/parentheses and titles, reference-style images via their definition (plain link definitions untouched), same-basename assets in different directories, assets above the source directory, unmatched parentheses in filenames and collection segments (reparsed to prove resolution), escaped definition labels, duplicate definitions |
...
| `scripts/live-check-vault.mjs` | Runs the live (network-dependent) rows against an explicitly designated throwaway vault, through the same publication-contract check `test/vault-capture-e2e.test.ts` uses (facts, one real MOC wikilink, `search` identity, full `read`), and prints a JSON summary; used by Task 7. `resolveGuardedState` (exported, round-4 addition) guard-checks the state directory and config path the same way as `--vault` before any write, and refuses to overwrite an existing `config.yaml` without `--overwrite-config`; round-5 adds `assertNotSymlink` calls on both the state dir and `config.yaml` before the containment check, rejecting a symlink outright (including a DANGLING one whose target does not exist and so cannot be containment-checked at all). Runs its CLI body only when invoked directly (`isMainModule` check), so `resolveGuardedState` can be imported for testing without triggering the argv-parsing/exit-on-missing-`--vault` path. |
| `scripts/live-check-vault.test.ts` | Unit contract for `resolveGuardedState`: throwaway-path acceptance (no writes), live-vault-itself/inside/symlink-alias rejection (including a not-yet-created symlinked child) before any write, existing-config refusal with bytes left untouched, `--overwrite-config` allowing it, the live-vault guard still winning over `--overwrite-config`, and (round 5) an external `config.yaml` symlinked to an EXISTING or NONEXISTENT file inside the live vault both rejected before any write |
| `scripts/lib/vault-guard.mjs` | `assertNotLiveVault`/`isInsideLiveVault` — canonicalizes both the live-vault path and the caller's destination (resolving the nearest existing ancestor for not-yet-created paths) so a symlink alias into the live vault cannot bypass the guard; handles an absent live vault explicitly instead of throwing. `isSymlinkPath`/`assertNotSymlink` (round-5 addition) `lstat` the candidate path itself (never following it) and reject any symlink outright, catching a DANGLING symlink whose target cannot be `realpathSync`-resolved at all |
| `scripts/lib/vault-guard.test.ts` | Unit contract for the guard: symlink-alias bypass, not-yet-created destination, absent live vault, a real child/symlink whose own name starts with `".."` (round-2 regression), and `isSymlinkPath`/`assertNotSymlink` against a plain file, a missing path, an existing-target symlink and a dangling symlink |
| `scripts/lib/qualification-contract.mjs` | `qualifyNote` — the one shared end-to-end publication-qualification contract (facts, frontmatter identity, whole-note digest, exactly one real MOC wikilink via `countMocLinksTo`, which delegates entirely to the shared, `remark`-based `src/vault/markdownLinks.mjs` — not a path substring match, and not a second independently-drifting fence stripper — `search` identity, full `read`), used by both `test/vault-capture-e2e.test.ts` and `scripts/live-check-vault.mjs` |
| `scripts/lib/qualification-contract.test.ts` | Unit contract for `qualifyNote`: a well-formed pass, and rejections for a missing fact, a wrong/missing/empty digest, missing `source_id`, a version mismatch, a wrong `source_url`, plain-text-only MOC mentions, a longer-prefix-sharing sibling target, malformed link syntax, a target only inside a backtick fence/tilde fence/inline code span/1-3-space-indented fence/4+-space-indented code block/multi-backtick span/multiline span/mismatched-delimiter-run span, duplicate links, an unresolved search, and an incomplete `read`; valid-single-link passes for an aliased wikilink, a real-link-plus-code-example, a bogus-closing-fence-with-trailing-text, a backtick-opener-with-backticks-in-its-info-string, and fence-looking-inline-text-with-no-match, each counting exactly one |
...
- `src/scraper/fetcher/FingerprintGenerator.ts` (and its test) — DOM settle rev 3 (2026-09-15), intentional shared behavior change: the default header pool is desktop-only (`devices: ["desktop"]`, `operatingSystems: ["windows", "linux", "macos"]`); callers may still pass mobile options explicitly, and a single-field override of `devices` or `operatingSystems` widens the paired half back to the full upstream pool so header-generator can still match (Codex diff review r1, major). Both `HttpFetcher` and `BrowserFetcher` draw from this generator, and a randomly drawn mobile user agent made Salesforce Experience Cloud serve a `formFactor: SMALL` bootstrap that never rendered in headless desktop Chromium, publishing the raw shell. This removes the random mobile draw; it does not make the browser fingerprint fully self-consistent (a Firefox or Safari header set can still accompany Chromium rendering). Pinned by `FingerprintGenerator.test.ts` (constructor constraints asserted through a `header-generator` spy, 100 sampled user agents, mobile negative control) and no-mobile-UA sampling in `HttpFetcher.test.ts` / `BrowserFetcher.test.ts`.
- `src/scraper/middleware/HtmlSanitizerMiddleware.ts` (and its test) — DOM settle rev 3: user `excludeSelectors` are removed *before* the empty-page safety-net snapshot, so the revert can only restore default boilerplate removals, never an explicitly excluded element. Default-selector behavior and the safety net itself are unchanged.
- `src/scraper/middleware/HtmlDefuddleMiddleware.ts` (and its test) — DOM settle rev 3: user `excludeSelectors` are applied to the source Cheerio DOM before `originalHtml` is read (replacing the linkedom pre-pass), so the low-retention, empty-output and exception fallbacks all hand back a DOM that already honours the exclusions.
- `src/vault-cli/commands/capture.ts` / `capture.test.ts` — DOM settle rev 3: repeatable `--exclude-selector <css>` forwarded verbatim into `ScraperOptions.excludeSelectors` (never comma-split — a comma is CSS grouping syntax). Content-only and per-invocation: `HtmlLinkExtractorMiddleware` runs before the extractor, so excluded elements still contribute crawl URLs, and nothing is remembered for the collection. Recaptures need the flag again.
- `src/scraper/middleware/MarkdownMetadataExtractorMiddleware.ts` (and its test) — 2026-09-15 skill test: additive third title fallback. When the served frontmatter is not valid YAML (an unquoted `title:` containing a colon, as higgsfield.ai serves) and the body has no H1, `recoverTitleFromRawFrontmatter` reads the raw `title:` line (matching quotes stripped) instead of leaving the note `Untitled`. It runs only when the YAML parser threw; valid YAML with a null/empty title still yields `Untitled` (Codex r1). Frontmatter-title and H1 precedence are unchanged. `src/vault/VaultCaptureService.ts` (fork file) pairs it with `stripLeadingFrontmatter` for Markdown sources.
