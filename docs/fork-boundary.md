...
| `src/vault/markdownLinks.mjs` | `countLinksTo({ markdown, target })`/`hasLinkTo({ markdown, target })` — the one shared Markdown wikilink-detection implementation, used by both `VaultPublisher.ts` and `scripts/lib/qualification-contract.mjs`. Round 6 (scoped) replaced the round-5 hand-rolled line-based fence/inline-span stripper (not CommonMark-correct: missed 1-3-space-indented fences, accepted a bogus closing-fence line with trailing text, accepted a backtick opener with backticks in its info string, mishandled multi-backtick/multiline spans and mismatched delimiter runs) with a real parse via the `remark`/`remark-parse`/`unified` toolchain already in the dependency tree (also used by `src/splitter/SemanticMarkdownSplitter.ts`; no new dependency): parses to an mdast tree, walks it (via a small local `walk()` -- not `unist-util-visit`, which is only a transitive dependency, not declared in `package.json`) skipping opaque nodes, and counts wikilinks in the remaining per-paragraph text. Round 7 (second scoped re-review) fixed two further defects: (1) every opaque node (`code`, `inlineCode`, `break`, `image`, `imageReference`, `html`, `footnote`, `footnoteReference`) now contributes a Unicode Private Use Area boundary character (`U+E000`) instead of `""`, so a wikilink fragmented by one can never be silently reassembled into a false match, while emphasis/strong/link nodes remain transparent for legitimate reassembly; (2) a substring fast path (`markdown.includes("[[" + target)`) plus a single-entry parse cache avoid reparsing a large MOC on every call (measured ~1.4s at 20k entries down to ~0.1ms when the target is absent), gated by `POSSIBLE_FRAGMENTING_MARKUP` so it never regresses the emphasis/strong/link reassembly guarantee. Root-level (block) `html` is deliberately never scanned -- `renderCollectionIndex` in `src/vault/render.ts` never emits it, so a link inside raw block HTML is out of scope, the same as inside a code fence. |
| `src/vault/markdownLinks.test.ts` | Unit contract for `countLinksTo`/`hasLinkTo`: plain/aliased link counting, the plain-text-syntax probe, emphasis-split reassembly, the 8 named CommonMark fence/span edge cases, duplicate-link counting, the boundary-sentinel fragmentation regressions (inline code, hard break, image, inline HTML), the block-HTML scope decision, structural coverage (headings, list items, blockquotes, a pipe-table line, strong/link reassembly, cross-block non-merging), and a benchmark-style large-flat-list-MOC performance test |
