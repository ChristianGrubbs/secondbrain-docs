...
| `src/vault/localAssets.ts` | `localizeLocalAssets` — pure rewrite of a `file://` source's relative Markdown image embeds to `_attachments/<collection folder>/<source_id[0:12]>/<basename>` plus the asset manifest the publisher copies first (Task 6 row F06) |
| `src/vault/localAssets.test.ts` | Unit contract for `localizeLocalAssets`: rewrite + manifest, untouched remote/absolute/`data:`/`.md`/missing targets, non-`file:` sources, dedupe and percent-decoding, directory-named target |
