...
| `src/vault/VaultPublisher.ts` | `VaultPublisher` — create / unchanged / replace (CAS) / conflict publication of one source note plus one MOC link, recovery of interrupted journal entries; `hasLinkTo` now delegates to the shared `src/vault/markdownLinks.mjs` |
| `src/vault/VaultPublisher.test.ts` | Unit contract for the publisher, including MOC-link regressions for backtick-fenced, tilde-fenced and inline-code-span mentions not counting as live links |
| `src/vault/markdownLinks.mjs` | `stripCodeAndInlineSpans`/`countLinksTo`/`hasLinkTo` — the one shared Markdown wikilink-detection implementation, used by both `VaultPublisher.ts` and `scripts/lib/qualification-contract.mjs`; strips backtick AND tilde code fences (respecting fence-closing length rules) plus inline code spans before counting `[[target]]`/`[[target\|alias]]` matches, so a code-example mention never counts as — or suppresses — a real link |
...
| `scripts/lib/qualification-contract.mjs` | `qualifyNote` — the one shared end-to-end publication-qualification contract (facts, frontmatter identity, whole-note digest, exactly one real MOC wikilink via `countMocLinksTo`, which delegates entirely to the shared `src/vault/markdownLinks.mjs` — not a path substring match, and not a second independently-drifting fence stripper — `search` identity, full `read`), used by both `test/vault-capture-e2e.test.ts` and `scripts/live-check-vault.mjs` |
| `scripts/lib/qualification-contract.test.ts` | Unit contract for `qualifyNote`: a well-formed pass, and rejections for a missing fact, a wrong/missing/empty digest, missing `source_id`, a version mismatch, a wrong `source_url`, plain-text-only MOC mentions, a longer-prefix-sharing sibling target, malformed link syntax, a target only inside a backtick fence/tilde fence/inline code span, duplicate links, an unresolved search, and an incomplete `read`; a valid aliased wikilink pass and a real-link-plus-code-example pass counting exactly one |
